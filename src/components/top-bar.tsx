...
export function TopBar() {
  ...
  const [conflictState, setConflictState] = useState<SyncConflictState | null>(
    null,
  );
  ...
  const handleConnect = async (url: string, apiKey: string) => {
    ...
    setConnectError(null);
    try {
      await UI.loading.showWhen(
        async () => {
          const { valid, tableMissing: missing } =
          ...
          const { remoteVersion, remoteTodos } =
            await readRemoteSnapshot(client);
  ...
  return (
    <React.Fragment>
      ...
      <Dialog
        ...
        disablePointerDismissal={loading === "sync"}
      >
        {conflictState ? (
...
async function readRemoteSnapshot(
  ...
  retries = 3,
): Promise<{ remoteVersion: number; remoteTodos: Todo[] }> {
